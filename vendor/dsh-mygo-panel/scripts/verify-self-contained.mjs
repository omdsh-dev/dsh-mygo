 ...
 * @deepseek-ai/*（AGENTS.md #2）；tsconfig references 不检查越界（#5）。
 */
...
if (!existsSync(join(root, 'src/README.md'))) failures.push('missing repository-layout contract src/README.md')
...
for (const field of ['dependencies', 'devDependencies', 'optionalDependencies', 'peerDependencies']) {
  for (const [name, spec] of Object.entries(packageJson[field] ?? {})) {
    ...
    if (/^workspace:/i.test(spec) && !name.startsWith('@deepseek-ai/')) {
