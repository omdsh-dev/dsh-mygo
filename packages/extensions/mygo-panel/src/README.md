...
`src/trust-fence.ts` / `src/hub-catalog.ts` / `src/hub-version.ts` 为
plughub 迁移面（trust fence / hub registry 投影与 spec 翻译 / semver 更新态）。
...
- `HubCatalog.tsx` — hub 目录页（P0 plughub 迁移：搜索/类型/风险筛选 +
  安装/更新 + operation SSE 状态与重启横幅）
