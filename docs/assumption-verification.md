...
| A9 entry 暴露 epoch/inertia | PARTIAL | entry.fiber 可达、inertia 是公开字段；epoch 在 lib 中仅存于私有 `_runner`，需公开 getter 或最小改动面 | eb-a09；entry.ts:56、fiber.ts:200、lib 无 epoch 字段 | EB-D14 边界 |
...
- **A9 修正了“fiber.epoch 可内省”的可行性**：inertia 公开可读，epoch 需私有
  访问或新增公开 getter（最小改动面：Fiber 增加 `get epoch()`）。
