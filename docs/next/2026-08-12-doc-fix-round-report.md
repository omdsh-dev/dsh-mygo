...
| A3 | 指控错（评审快照过时） | 实现：args.ts:2/73 手写解析，无 commander import；design-r5-cli.md:350-351 + Rev-A2（:495②） | 无新动作（偏离已在 Rev-A2 登记） |
| A4 | 指控错（评审快照过时） | verify-self-contained.mjs:100（lockfile 必读）、:125（恰 7 skills）；design-r5-cli.md:379-384 + Rev-A2（:495③，含推翻「不复制」的原因） | 无新动作（已在 Rev-A2 登记） |
| B1 | 偏离未登记 → 已登记 | resolver.ts:77-88；design-r4-backlog.md:36-37 | design-r4.md §13 追加 Rev-P4（:318） |
| B2 | 指控错（评审快照过时） | cli-verification.md §8.6 需求 2（:209-214，c289c7b 已补登）；DEV-GUIDE §10 一致 | 无新动作 |
...
- 判定：**指控错（评审快照过时）**——评审声称 design-r5 写 commander，实为
  Rev-A2 之前的旧文本；偏离登记在评审前已完成，无新 Rev 需求。
