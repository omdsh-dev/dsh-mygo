...
| 担保项 | 体系内插件 | 社区插件 |
|---|---|---|
| 安装期求解 / lockfile | ✅（mygo 控制面） | —（npm/pnpm 原生解析） |
| depends/breaks 硬阻断 | ✅（插件图约束） | —（仅 npm peer 告警，见 §9） |
| 符号前置门（细 epoch / reload 校验） | ✅ | — |
| exports 冻结 / Proxy 包装 | ✅（桥接路径强制，EB-D8） | —（契约外行为，后果自负） |
| 运行期反应式启停 | ✅（原生惯性 + mygo 政策闸） | ✅（原生反应式直接享有） |
| 状态观察 / 报告可见 | ✅（BOM 对账 + fiber 内省 + 结构化报告） | ✅（只读观察：fiber 状态、BOM 版本/peer 告警） |

> 社区侧仅有“运行期反应式启停”与“状态观察/报告可见”两项为 ✅，其余不担保。
