...
|---|---|---|---|---|---|---|
| 版本升级 + 方法改名（B 未迁移） | A 重装 0.0.1→0.0.2，exports {A}→{A',B} | ✅ uid 变 | `{A}⊄{A',B}` 失败 | 已删除 | A、B 整体回滚旧版 + 硬告警/报告 | B INACTIVE + 报告 |
| 版本升级 + 方法改名（B 已迁移） | 同上，B 声明用 A' | ✅ uid 变 | `{A'}⊆{A',B}` 通过 | —（成功） | — | B ACTIVE |
| 新增符号 B | A 更新加入 B | ✅ uid 变 | 投影外消费者无变化；投影内 C 通过 | — | — | C ACTIVE；无关者不动 |
| 版本不变静默替换（重装） | A 同版本新模块 | ✅ uid 变 | 符号校验可抓 | 已删除 | 回滚旧模块 + 硬告警/报告 | B INACTIVE + 报告 |
| 版本不变同实例原地改导出 | A 对象上 A→A'，不重装 | ❌ uid 不变、无 notify | 原生全盲；需结构性比较/替换契约 | — | — | 不可见（未解） |
